...
const CaseStudyCriteriaManage = () => {
    const urlParams = new URLSearchParams(window.location.search);
    const criteriaId = urlParams.get('n');
    const caseStudyId = urlParams.get('c');
    const { save, setInputValue, changeLanguage,
        ...
        language,
        redirect
    } = Logic(criteriaId, caseStudyId);

    if (redirect) {
        window.location.replace("case.study.manage?n=" + caseStudyId + "&l=" + language);

        return;
    }
    return (
        <>       
            <MenuForm title={title} menus={[{ url: "case.study.manage?n=" + caseStudyId + "&l=" + language, title: "List", icon: "fa fa-arrow-left" }]} />
            {
...
export default CaseStudyCriteriaManage;